...
func (m *systemdDaemonManager) StartDaemon(name string) error {
	...
	_, err := m.conn.StartUnit(unitName, ModeReplace, nil)
	return err
}

func (m *systemdDaemonManager) StopDaemon(name string) error {
	unitName := getServiceUnitName(name)
	_, err := m.conn.StopUnit(unitName, ModeReplace, nil)
	return err
}

func (m *systemdDaemonManager) RestartDaemon(name string) error {
	unitName := getServiceUnitName(name)
	_, err := m.conn.RestartUnit(unitName, ModeReplace, nil)
	return err
}

func (m *systemdDaemonManager) GetDaemonStatus(name string) (DaemonStatus, error) {
	unitName := getServiceUnitName(name)
	status, err := m.conn.GetUnitProperty(unitName, "ActiveState")
	if err != nil {
...
func (m *systemdDaemonManager) EnableDaemon(name string) error {
	unitName := getServiceUnitName(name)
	_, changes, err := m.conn.EnableUnitFiles([]string{unitName}, false, false)
	if err != nil {
		return err
	}
	if len(changes) != 1 {
...
func (m *systemdDaemonManager) DisableDaemon(name string) error {
	unitName := getServiceUnitName(name)
	changes, err := m.conn.DisableUnitFiles([]string{unitName}, false)
	if err != nil {
		return err
	}
	if len(changes) != 1 {
